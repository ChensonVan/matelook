...
class MailUtils():
    ...
    def reset(self, zid, code=next_id()):
        ...
        content += 'http://%s:%s/account/reset?zid=%s&&resetCode=%s' % (_URL_, _PORT_, zid, code)
        # print(content)
        msg = MIMEText(content)
        msg['From'] = self._format_addr('{} <{}>'.format(self.sender, self.from_addr))
        msg['To'] = self._format_addr('{} <{}>'.format(self.receiver, self.to_addr))
    ...
    def notificaion(self):
        content = 'Hello {}, you hava a new notification, please check your account.'.format(self.receiver)
        msg = MIMEText(content)
        msg['From'] = self._format_addr('{} <{}>'.format(self.sender, self.from_addr))
        msg['To'] = self._format_addr('{} <{}>'.format(self.receiver, self.to_addr))
